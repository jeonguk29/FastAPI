...
'''
유저 메모 테이블 관계 만들고 
로그인된 유저는 메모 생성, 수정, 삭제, 조회 가능하게 만들기
'''
class User(Base):
    __tablename__ = "users"
...
class Memo(Base):
    __tablename__ = 'memo'
    id = Column(Integer, primary_key=True, index=True) 
    user_id = Column(Integer, ForeignKey('users.id')) # 유저 id랑 연결 
...
async def create_memo(request: Request, memo: MemoCreate, db: Session = Depends(get_db)):
    ...
    if username is None: # 유저 이름이 없다면 로그인이 안되어 있는 것임
        raise HTTPException(status_code=401, detail="Not authorized")
    user = db.query(User).filter(User.username == username).first()
    if user is None: # 서버에서 해당 유저 이름이 없다면
...
async def list_memos(request: Request, db: Session = Depends(get_db)):
    ...
    #유저가 작성한 메모만 보여주기 (리턴을 html 형식으로)
