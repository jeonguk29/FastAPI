...
from passlib.context import CryptContext # 패스워드 변환 저장시 많이 사용하는 라이브러리
from starlette.middleware.sessions import SessionMiddleware


'''
1. 로그인이 된후, 접속이 가능한 페이지 SessionMiddleware
2. 회원가입시 등록한 패스워드를 어떻게 저장할 것이냐?
    - plain text를 해슁 기법을 사용해서, 알아볼 수 없는 데이터로 저장
    - 패스워드 입력시, 동일 해슁기법으로 변환 후, 데이터 베이스의 데이터로 확인 
    (plain text)가 동일하면 동일한 값으로 변환 될것임
'''
pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto") # bcrypt는 패스워드 변환시 가장 많이 쓰는 알고리즘 해싱 기법중 하나

def get_password_hash(password):
    return pwd_context.hash(password)

# 로그인 할때 입력한 비번, DB에서 가져온 비번
